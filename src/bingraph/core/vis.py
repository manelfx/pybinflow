...
class Node:
    def __init__(self, obj: CFGNode) -> None:
        self.obj = obj
...
class Edge:
    def __init__(self, src: Node, dst: Node, meta: dict[str, Any] | None = None) -> None:
        self.src = src
        self.dst = dst
        ...
        for annotator in self.annotators:
            if isinstance(annotator, NodeAnnotator):
                ...
                if annotator.name not in self._contents:
                    raise VisError(f"Content '{annotator.name}' not found, required by annotator '{type(annotator)}'")
