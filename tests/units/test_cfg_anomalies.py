...
from bingraph.cfg import repair
...
def test_decoding_coverage_detects_zero_sized_nodes() -> None:
    ...
    anomaly = repair._decoding_coverage_anomaly(SimpleNamespace(addr=0x1000, size=0))
...
def test_decoding_coverage_accepts_exact_capstone_span(
    ...
    assert (
        repair._decoding_coverage_anomaly(SimpleNamespace(addr=0x1000, size=2)) is None
    )
...
def test_anomaly_detector_logs_each_kind_and_address_once() -> None:
    ...
    try:
        detector = repair.CFGAnomalyDetector(
            object(),
            object(),
